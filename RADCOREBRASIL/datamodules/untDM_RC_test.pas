...
begin
  ...
  if ( pSQLMaster.State in [ dsEdit, dsInsert ] ) then //or ( bOnlySearch ) then // v. 4.0.0.0 onlysearch
  begin
     try
        // se teclou ESC no LOOKUP, nao atualiza os dados
        if ( mm.varC_Lookup_Code <> '_ESC_' ) then
...
//                 end;
                 ...
                 cTemp := Copy( cTemp, 1, Pos( ' ', cTemp ) -1 );
                 dm_rc.rc_FillComboBox( TComponent(pFrame).Components[I] , '','','', cTemp );
              end;
