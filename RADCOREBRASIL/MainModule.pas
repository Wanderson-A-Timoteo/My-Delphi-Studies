...
uses
  ...
  untdm_rc,
  Main, Login, mkm_layout;//untDM_LOOKUPs,  untDM_TOTALS;
...
begin
     ...
     // verificar se, por algum descuido, a pasta onde foi gerado o EXE/DLL não contém os arquivos padrões do RadCORE
     if not DirectoryExists( APP_PATH_FILES_GLOBAL ) then
     begin
          varB_RCFoldersMissing := true;
     end;
     if not DirectoryExists( APP_PATH_CSS_GLOBAL ) then
     begin
          varB_RCFoldersMissing := true;
     end;
     if not DirectoryExists( APP_PATH_JS_GLOBAL ) then
     begin
          varB_RCFoldersMissing := true;
     end;
     if not varB_RCFoldersMissing then
     begin
         // v. 3.3.0.0
