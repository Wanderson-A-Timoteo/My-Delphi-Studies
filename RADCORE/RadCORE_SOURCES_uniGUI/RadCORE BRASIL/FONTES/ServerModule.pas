unit ServerModule; // v. 4.0.0.0

interface

uses
  Classes, SysUtils, uniGUIServer, uniGUIMainModule, uniGUIApplication, uIdCustomHTTPServer, // v. 4.0.0.0
  ...
  FireDAC.Phys.IBBase, FireDAC.Phys.FB, FireDAC.Comp.UI, FireDAC.ConsoleUI.Wait, Vcl.ExtCtrls, Vcl.Menus;
...
uses
  UniGUIVars, uconsts, MainModule, untdm_rc, str_func, mkm_func_web, uVersion;
...
procedure Tsm.Option11Click(Sender: TObject);
begin
   // se quiser abrir o projeto ao executar, descomente a linha
   {$ifdef MSWINDOWS}
   ExploreWeb( PChar( 'http://localhost:' + Port.ToString ) );
...
procedure Tsm.Option31Click(Sender: TObject);
begin
...
begin
  ...
  //test....
  if ARequestInfo.URI = '/getLicence' then
  begin
    AResponseInfo.ContentText := 'Client has 5 access licences.';
...
initialization
  ...
  //
  // Comment this block below if you are outside Brazil
  //
