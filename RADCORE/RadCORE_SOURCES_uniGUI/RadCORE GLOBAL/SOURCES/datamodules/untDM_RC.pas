        ...
        begin
            ...
            if LkpEDIT <> nil then
               ...
               begin
                  ...
                  cFrm := Trim( Copy( cFrm , 1, Pos( '|', cFrm )-1 ) );                     // v. 3.2.0.4
            ...
            begin
               ...
               MainForm.rc_AddFormFrameInTab( nil, '', cTableName + '_ADD', cFrm , false, false );
            end;
        end;
end;
// v. 4.0.0.0
                 ...
                 begin
                      ...
                      begin
                           ...
                           begin
                              ...
                              begin
                                   cTemp3 := trim( Copy( cTemp, Pos( 'mobile-v:', cTemp ) + 9 ) ) + ' ';
                                   cTemp3 := trim( Copy( cTemp3, 1, Pos( ' ', cTemp3 ) ) );
                                   cTemp  := cTemp3;
                                   if Pos( 'off', cTemp3 ) > 0 then
                                   begin
                                      cTemp  := '1%';
                                      cTemp3 := '1';
                                      cvisible := 'false';
...
begin
     ...
     (*<prj-nologin*)frmLogin.btnLog_In.ModalResult := mrNone;(*prj-nologin>*)
     ...
     begin
         ...
         begin
             ...
             (*<prj-nologin*)frmLogin.edUserName.SetFocus;(*prj-nologin>*)
             Exit;
         end;
         ...
         begin
             ...
             begin
                   ...
                   (*<prj-nologin*)frmLogin.edPassword.SetFocus;(*prj-nologin>*)
                   Exit;
             end;
         end;
          ...
          (*<prj-nologin*)bRemember := frmLogin.cbRemember.Checked;(*prj-nologin>*)
          if bRemember then
          begin
...
begin
  ...
  begin
     ...
     If Pos( ' #', LkpDBEdit.Hint ) > 0 then
     begin
        sExtraReturnField := Copy( LkpDBEdit.Hint, Pos( ' #', LkpDBEdit.Hint ) + 2 ) + '| ';
        ...
        // another attribute after "#".."="
        if Pos( ' ', sExtraReturnField ) > 0 then
           sExtraReturnField := Copy( sExtraReturnField, 1, Pos( ' ', sExtraReturnField ) - 1 ) + '| ';
        mm.varC_Temp1 := sExtraReturnField; // remove..is test
        If Pos( '=', sExtraReturnField ) > 0 then
        begin
           sExtraReturnField := Copy( sExtraReturnField, Pos( '=', sExtraReturnField ) + 1 )+ '|';
           mm.varC_Temp1 := sExtraReturnField; // remove..is test
           sExtraReturnField := Copy( sExtraReturnField, 1, Pos( '|', sExtraReturnField ) - 1 );
        end
        else
...
//            ( pSQLDetail.FieldByName( sExtraSearchField ).AsString = '0' ) then
  ...
  begin
     ...
     sSearchFields := varIIF( sExtraReturnField <> '', 'tab.' + sExtraReturnField, '' ); // v. 4.1.0.0
...
begin
  inherited;
    ...
    begin
      ...
      //cbx-fill:ufs codigo+descricao save:descricao
            ...
            try
               ...
               begin
                  ...
                  begin
                      ...
                      // v. 4.1.0.0
                      // concatenar campos
                      if ( strTokenCount( LkpDBEdit.Hint , '+' ) > 1 ) then
                      begin
                           iPos := strTokenCount( LkpDBEdit.Hint , '+' );
                           LkpName_DataField_Concat := LkpDBEdit.Hint;
                           // remover parcial se for o "+" para botão ADD
                           // remove partial if it is the "+" for ADD button
                           iPos := Pos( ' +', LkpName_DataField_Concat );
                           if iPos > 0 then
                           begin
                              LkpName_DataField_Concat := Trim( Copy( LkpName_DataField_Concat, 1, iPos-1 ) ) + ' ';
                           end;
                           iPos := Pos( '+', LkpName_DataField_Concat );
                           if iPos > 0 then
                           begin
                              LkpName_DataField_Concat := Trim( Copy( LkpName_DataField_Concat, iPos + 1, 100 ) );
...
begin
    ...
    begin
       cText := TUniComboBox( pCbx ).Text;
       cTemp := pFiles;// sm.FilesFolderPath + 'translate' + BACKSLASH + cTemp ;

       TUniComboBox( pCbx ).Items.Clear;
       TUniComboBox( pCbx ).ItemIndex := -1;
       I := FindFirst( cTemp + BACKSLASH + '*.gif' , faAnyFile, vSR );
       while I = 0 do
       begin
         TUniComboBox( pCbx ).Items.Add( vSR.Name ); //ChangeFileExt(vSR.Name, '' )
         I := FindNext( vSR );
       end;
       I := FindFirst( cTemp + BACKSLASH + '*.svg' , faAnyFile, vSR );
       while I = 0 do
       begin
