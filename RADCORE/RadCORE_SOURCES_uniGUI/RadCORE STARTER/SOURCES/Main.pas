...
uses
  ...
  uniGUIDialogs,
  Soap.SOAPHTTPClient, //uniGUIRegClasses,
  ...
  uniDBGrid, uniCanvas, uniMemo, uniDateTimePicker, uniDBComboBox, uniDBDateTimePicker, UniDBEdit, uniDBLookupComboBox,
  uniCheckBox, uniRadioGroup, uniDBMemo, uniDBNavigator, uniDBImage,
  Vcl.Menus, uniMainMenu, // v. 4.0.0.0
  uniSpeedButton, uniRadioButton, Vcl.Imaging.jpeg; // v. 3.0.0.0
...
//      OldHeight: Integer);
    ...
    procedure UniScrollBox1AjaxEvent(Sender: TComponent; EventName: string; Params: TUniStrings);
...
//  mm.MNU_STATE  := paLayoutMainMenu.tag.ToString;
  ...
  Self.Script.Text :=
      ...
      // v. 3.3.0.1
      '}' +
      'function rc_scrollToBottom (id,offset) {'+
      '   var div = document.getElementById(id);'+
      '   div.scrollTop = ( div.scrollHeight - div.clientHeight ) + offset;'+
      '}' +
      'function rc_scrollToTop (id) {'+
      '   var div = document.getElementById(id);'+
      '   div.scrollTop = 0;'+
      '}' +
      ...
      '  }, 350);' +
      '}' +
      'function rc_mobileEditEnter (id,value) {'+
      '   var div = document.getElementById(id);'+
      '   div.clientHeight = div.clientHeight + value;'+
      '}' +
      'function rc_mobileEditExit (id,value) {'+
      '   var div = document.getElementById(id);'+
      '   div.clientHeight = div.clientHeight - value;'+
      '}';
     ...
     //  UniSession.JSCode('$("#' + uniedit2.JSName + '_id-inputEl").inputmask("99999-9999");');
     //uniedit2.InputType := 'tel';
...
procedure TMainForm.UniScrollBox1AjaxEvent(Sender: TComponent; EventName: string; Params: TUniStrings);
begin
...
begin
    ...
    begin
       ...
       if oObj2 <> nil then
       begin
          TUnicontrol( oObj2 ).Left      := 0;//TUnicontrol( oObj1 ).Width; // v. 4.0.0.0
          TUnicontrol( oObj2 ).Height    := Self.Height;
          TUnicontrol( oObj2 ).Width     := Self.Width;// v. 4.0.0.0;// - TUnicontrol( oObj1 ).Width;
       end;
    end
    else
    begin
