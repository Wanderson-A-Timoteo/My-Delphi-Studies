...
begin
  ...
  if not FileExists(ARQUIVOCONFIGURACAO) then
    raise Exception.Create('Error: Arquivo de configuração do Banco de Dados não encontrado.');
  ...
  try
    ...
    begin
      ...
      begin
        ParametroNome  := ListaParametros[Contador].Split(['='])[0].Trim; // Trim retira os espaços
